...
mfi_car <- read_xlsx("Car/MFI_CAR_2021.xlsx")%>% rename(
  Admin1NAME = TrdNodDensLocNameAdm1, Admin2NAME = TrdNodDensLocNameAdm2)

marche_car <- read_xlsx("Car/Marche_car.xlsx")

# mfi_car <- mfi_car %>% mutate(
#   Admin1name = VLOOKUP(.lookup_values = ADMIN1Name, .data = marche_car,.lookup_column = Adm1Code, .return_column = Adm1Name )
# )
...
tableauAdmin2 <- function(d, colonne){
  d <- mfi_bissau %>% tabyl(Admin2NAME, {{colonne}},show_na = TRUE) %>% 
    ...
    adorn_percentages(denominator = "row") %>% 
    adorn_pct_formatting(digits = 0,affix_sign = FALSE) %>% 
    left_join(TableDeRechercheAdmin2, by = "Admin2NAME") %>% 
    relocate(Admin1NAME,.before = Admin2NAME)
...
tableauMarche <- function(d,colonne){
  d <- mfi_bissau %>% tabyl(MktNametext, {{colonne}},show_na = TRUE) %>% 
    ...
    adorn_percentages(denominator = "row") %>% 
    adorn_pct_formatting(digits = 0,affix_sign = FALSE) %>% 
    left_join(TableDeRechercheMarche, by = "MktNametext") %>% 
    select(Admin1NAME, Admin2NAME,MktNametext,everything())
}
...
BaseAdmin1 <- c(AssortimentAdmin1,DisponibiliteAdmin1,PrixAdmin1,ResilienceAdmin1)

BaseAdmin2 <- c(AssortimentAdmin2, DisponibiliteAdmin2,PrixAdmin2,ResilienceAdmin2)

BaseMarche <- c(AssortimentMarche, DisponibiliteMarche, PrixMarche,ResilienceMarche)
...
saveWorkbook(wb, "Car/Admin1.xlsx")
...
saveWorkbook(wb, "Car/Admin2.xlsx")
...
saveWorkbook(wb, "Car/Marche.xlsx")